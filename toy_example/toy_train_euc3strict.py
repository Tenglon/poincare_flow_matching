...
traj_dir = "trajectory_euc3strict"
...
# Calculate angles for all centers to find well-distributed sources
centers_angles = np.arctan2(centers[:, 1], centers[:, 0])
centers_angles = (centers_angles + 2 * np.pi) % (2 * np.pi)  # Normalize to [0, 2π]

# Parent class groups in shuffled positions
# Parent class 1: shuffled positions [2, 7, 11] (original: 1, 0, 2)
# Parent class 2: shuffled positions [1, 5, 9] (original: 3, 5, 4)
# Parent class 3: shuffled positions [3, 8, 10] (original: 7, 8, 6)
# Parent class 4: shuffled positions [0, 4, 6] (original: 10, 11, 9)
parent_groups = [
    [2, 7, 11],  # Parent class 1
    [1, 5, 9],   # Parent class 2
    [3, 8, 10],  # Parent class 3
    [0, 4, 6],   # Parent class 4
]

# Select well-distributed sources from each parent class
# Strategy: Choose the most extreme angle position in each group as source
# This ensures sources are maximally spread in angle space
selected_pairs = []
for i, group in enumerate(parent_groups):
    # Calculate angles for this group
    group_angles = [centers_angles[pos] for pos in group]
    # Find the position with the most extreme angle (min or max)
    # Alternate between choosing min and max to ensure good distribution
    if i % 2 == 0:
        # Choose minimum angle (leftmost)
        source_idx = group[np.argmin(group_angles)]
    else:
        # Choose maximum angle (rightmost)
        source_idx = group[np.argmax(group_angles)]
    
    # Choose a different position in the same group as target
    target_candidates = [pos for pos in group if pos != source_idx]
    # Choose the middle position if available, otherwise first available
    if len(target_candidates) > 1:
        # Choose the one with angle closest to group mean
        group_mean = np.mean(group_angles)
        target_angles = [centers_angles[pos] for pos in target_candidates]
        mean_distances = [abs(angle - group_mean) for angle in target_angles]
        target_idx = target_candidates[np.argmin(mean_distances)]
    else:
        target_idx = target_candidates[0]
    
    selected_pairs.append((source_idx, target_idx))
    
    # Verify that source and target are in the same parent class
    assert source_idx in group and target_idx in group, \
        f"Pair ({source_idx}, {target_idx}) must be in the same parent class group {group}"

# Verify all pairs are within the same parent class
print("Verifying all pairs are within the same parent class...")
for pair in selected_pairs:
    ...
    assert source_group == target_group, \
        f"Pair ({source}, {target}): source in parent {source_group}, target in parent {target_group}"
    print(f"  Pair ({source}, {target}): Both in parent class {source_group + 1} ✓")
...
# Print actual source_inx and target_inx for debugging
print(f"source_inx (shuffled positions): {source_inx.tolist()}")
print(f"target_inx (shuffled positions): {target_inx.tolist()}")

# Source and target orders in shuffled positions
# Will be set based on dynamically selected pairs
source_order_shuffled = [item[0] for item in selected_pairs]  # Source positions from pairs
target_order_shuffled = [item[1] for item in selected_pairs]  # Target positions from pairs

# Create a mapping to verify pairs are in same parent class
def get_parent_class(pos):
    """Get parent class index for a given shuffled position"""
    for i, group in enumerate(parent_groups):
        if pos in group:
            return i
    return None

# Verify all selected pairs are in the same parent class
def verify_pair_in_same_parent(source_pos, target_pos):
    """Verify that source and target are in the same parent class"""
    source_parent = get_parent_class(source_pos)
    target_parent = get_parent_class(target_pos)
    if source_parent != target_parent:
        raise ValueError(f"Pair ({source_pos}, {target_pos}): source in parent {source_parent}, target in parent {target_parent}")
    return source_parent

# Verify initial pairs
for source, target in selected_pairs:
    verify_pair_in_same_parent(source, target)
...
for k in range(10000):
    ...
    # Strict verification: ensure all pairs are in the same parent class
    # y0 contains source labels, y1 contains target labels
    # Each label should correspond to a position in selected_pairs
    if k == 0:  # Only verify on first iteration to avoid performance impact
        for i in range(len(selected_pairs)):
            source_pos = selected_pairs[i][0]
            target_pos = selected_pairs[i][1]
            verify_pair_in_same_parent(source_pos, target_pos)

    # Sort using shuffled positions
    # Create a mapping for sorting that handles missing values
    def get_sort_key_source(val):
        if val.item() in source_order_shuffled:
            return source_order_shuffled.index(val.item())
        else:
            return len(source_order_shuffled)  # Put missing values at the end
    
    def get_sort_key_target(val):
        if val.item() in target_order_shuffled:
            return target_order_shuffled.index(val.item())
        else:
            return len(target_order_shuffled)  # Put missing values at the end
    
    sorted_pairs_y0 = sorted(enumerate(y0), key=lambda x: get_sort_key_source(x[1]))
    sorted_pairs_y1 = sorted(enumerate(y1), key=lambda x: get_sort_key_target(x[1]))
